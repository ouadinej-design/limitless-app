...
const CHECKLIST0 = [
  { id:1,  text:"Regarder la vidéo de bienvenue de Marie" },
  { id:2,  text:"Créer ton espace consultant sur chogan.eu" },
  { id:3,  text:"Commander ta mallette de démarrage" },
  { id:4,  text:"Rejoindre le groupe Telegram de l'équipe" },
  { id:5,  text:"Télécharger les modèles Canva" },
  { id:6,  text:"Créer ta page Instagram / Facebook Chogan" },
  { id:7,  text:"Publier ta première story de présentation" },
  { id:8,  text:"Maîtriser le script de contact (Module 2)" },
  { id:9,  text:"Effectuer ta première vente" },
  { id:10, text:"Recruter ton premier filleul" },
  { id:11, text:"Réussir le quiz de certification (8/10 min.)" },
  { id:12, text:"Atteindre le statut Gold" },
].map(i => ({ ...i, done: false }));
...
const QUIZ = [
  { id:1,  q:"Quelle couleur de texte indique un parfum FEMME ?",   opts:["Noir","Vert","Rose","Bleu"],    correct:2 },
  { id:2,  q:"Quelle couleur de texte indique un parfum HOMME ?",   opts:["Rouge","Vert","Rose","Noir"],   correct:3 },
  { id:3,  q:"Quelle couleur de texte indique un parfum MIXTE ?",   opts:["Rose","Vert","Noir","Violet"],  correct:1 },
  { id:4,  q:"Quelle est la concentration en parfum Chogan ?",       opts:["15%","20%","30%","40%"],        correct:2 },
  { id:5,  q:"Combien de références au catalogue Chogan ?",          opts:["50+","100+","157+","200+"],     correct:2 },
  { id:6,  q:"Quel score minimum pour la certification ?",           opts:["6/10","7/10","8/10","10/10"],   correct:2 },
  { id:7,  q:"Quelle est la plus petite contenance disponible ?",    opts:["10ml","15ml","20ml","25ml"],    correct:1 },
  { id:8,  q:"Comment contacter le support Gold ?",                  opts:["Email","Formulaire","WhatsApp direct","Telegram"], correct:2 },
  { id:9,  q:"Quelle plateforme pour les modèles de posts ?",        opts:["Pinterest","Canva","Instagram","Figma"],           correct:1 },
  { id:10, q:"1ère étape après ton inscription ?",                   opts:["Recruter","Commander mallette","Vidéo bienvenue","Créer réseaux"], correct:2 },
];
...
.app{max-width:430px;margin:0 auto;min-height:100vh;display:flex;flex-direction:column;overflow-x:hidden;}
.hdr{background:#0a0a1a;border-bottom:0.5px solid rgba(201,168,76,.18);padding:13px 18px;display:flex;align-items:center;position:sticky;top:0;z-index:100;}
.logo{font-family:'Cormorant Garamond',Georgia,serif;font-size:20px;font-weight:700;color:${G};letter-spacing:4px;text-transform:uppercase;}
.hdr-sub{margin-left:auto;font-size:10px;color:${MU};letter-spacing:2px;text-transform:uppercase;}
.main{flex:1;padding-bottom:82px;overflow-y:auto;}
.bnav{position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:430px;background:#0a0a1a;border-top:0.5px solid rgba(201,168,76,.12);display:flex;z-index:200;}
.nb{flex:1;border:none;background:transparent;color:${MU};padding:9px 2px 7px;display:flex;flex-direction:column;align-items:center;gap:2px;cursor:pointer;transition:color .15s;font-family:'DM Sans',sans-serif;}
.nb.on{color:${G};}
.nic{font-size:17px;line-height:1;}
.nlbl{font-size:8px;letter-spacing:.5px;text-transform:uppercase;}
...
function AccueilView({ started, setStarted }) {
  ...
  if (!started) return (
    <div className="fi" style={{ minHeight:"calc(100vh - 130px)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"32px 22px", textAlign:"center", background:"radial-gradient(ellipse at 50% 25%, rgba(201,168,76,.07) 0%, transparent 65%)" }}>
      <div style={{ fontSize:44, marginBottom:14 }}>✦</div>
      <div style={{ fontFamily:"'Cormorant Garamond',Georgia,serif", fontSize:30, fontWeight:700, color:G, lineHeight:1.2, marginBottom:6 }}>Bienvenue chez Chogan</div>
      <p style={{ fontSize:12, color:MU, marginBottom:22 }}>Ton espace consultant personnel</p>
      <div style={{ background:"rgba(255,255,255,.025)", border:"0.5px solid rgba(201,168,76,.2)", borderRadius:14, padding:"20px 20px 16px", marginBottom:24, textAlign:"left", position:"relative" }}>
        <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:56, color:G, opacity:.18, position:"absolute", top:-8, left:12, lineHeight:1 }}>"</div>
        <p style={{ fontSize:13, lineHeight:1.8, color:"#ccc", paddingTop:14 }}>
          Chère consultante, je suis tellement heureuse de t'accueillir dans notre équipe ! 🌸<br/><br/>
          Cette application va t'accompagner pas à pas dans ton aventure Chogan — de ta première vente jusqu'au statut Platinium et au-delà.<br/><br/>
          Suis ta checklist, explore le catalogue complet, et utilise nos scripts éprouvés pour contacter tes prospects avec confiance.<br/><br/>
          Tu n'es pas seule — toute l'équipe est là pour toi ! ✨
        </p>
        <p style={{ fontSize:13, color:G, fontFamily:"'Cormorant Garamond',serif", fontStyle:"italic", marginTop:12 }}>— Marie</p>
      </div>
    ...
    <div className="fi">
      ...
      <div className="sb">
        ...
        <div style={{ display:"flex", gap:8, marginTop:6 }}>
          <input className="inp" placeholder="Nouvelle annonce…" value={newA} onChange={e=>setNewA(e.target.value)} style={{ flex:1 }} />
          <button className="btn-o" onClick={()=>{ if(!newA.trim())return; setAnns(p=>[...p,{id:Date.now(),text:newA,date:new Date().toLocaleDateString("fr-FR")}]); setNewA(""); }}>+</button>
        </div>
      </div>
      ...
      <div className="sb">
        ...
        <div style={{ display:"flex", gap:8, marginTop:6 }}>
          <input className="inp" placeholder="Prénom + succès…" value={newS} onChange={e=>setNewS(e.target.value)} style={{ flex:1 }} />
          <button className="btn-o" onClick={()=>{
            if(!newS.trim())return;
            const [name,...rest]=newS.split(" ");
            setSucs(p=>[...p,{id:Date.now(),name,ach:rest.join(" ")||"Nouveau succès 🎉"}]);
            setNewS("");
          }}>+</button>
        </div>
      </div>
...
function BonCommandeView({ perfumes }) {
  ...
  const totalDzd = Math.round(totalEur * tauxN + (pays==="dz"?fraisN:0));
  const totalEurFinal = totalEur + (pays==="fr"?fraisN/tauxN:0);
  ...
  const exportText = () => {
    ...
    if(pays==="dz") { lines.push(`💱 Taux du jour : 1€ = ${taux} DA`); lines.push(`🚚 Frais envoi : ${frais} DA`); lines.push(`💰 TOTAL DZD : ${totalDzd.toLocaleString("fr-FR")} DA`); }
    if(pays==="fr") lines.push(`🚚 Frais de port : ${fraisN} €`);
        ...
        {pays==="dz" && (
          <div style={{ display:"flex", gap:8, marginBottom:14 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <input className="inp" type="number" value={frais} onChange={e=>setFrais(e.target.value)} placeholder="500" />
            </div>
          </div>
        )}
        {pays==="fr" && (
              ...
              <div className="cardg" style={{ marginTop:10 }}>
                ...
                {pays==="fr" && (
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                    ...
                    <span style={{ fontSize:13, fontWeight:600 }}>{parseFloat(frais)||0} €</span>
                  </div>
                )}
                {pays==="dz" && (
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                    <span style={{ fontSize:12, color:MU }}>Frais d'envoi</span>
                    <span style={{ fontSize:13, fontWeight:600 }}>{parseFloat(frais)||0} DA</span>
                  </div>
                )}
                ...
                {pays==="fr" && (
                  <div style={{ display:"flex", justifyContent:"space-between" }}>
                    ...
                    <span style={{ fontSize:22, fontWeight:700, color:G }}>{(totalEur+(parseFloat(frais)||0)).toFixed(2)} €</span>
                  </div>
                )}
                {pays==="dz" && (
                  <>
...
const PRODUITS_PROMO = [
  { id:"etuis",  nom:"Étuis Simple",  prixEur:9.10,  transport:1.80, emballage:0 },
  { id:"cuir",   nom:"Étui Cuir",     prixEur:10.50, transport:1.80, emballage:0 },
  { id:"15ml",   nom:"15 ML",         prixEur:5.95,  transport:1.80, emballage:0.48 },
  { id:"30ml",   nom:"30 ML",         prixEur:9.00,  transport:1.80, emballage:0.48 },
  { id:"70ml",   nom:"70 ML",         prixEur:18.00, transport:1.80, emballage:0.48 },
  { id:"lux1",   nom:"Luxury 22,50€", prixEur:22.50, transport:1.80, emballage:0.48 },
  { id:"lux2",   nom:"Luxury 23€",    prixEur:23.00, transport:1.80, emballage:0.48 },
  { id:"lux3",   nom:"Luxury 24€",    prixEur:24.00, transport:1.80, emballage:0.48 },
  { id:"lux4",   nom:"Luxury 26€",    prixEur:26.00, transport:1.80, emballage:0.48 },
  { id:"lux5",   nom:"Luxury 28,50€", prixEur:28.50, transport:1.80, emballage:0.48 },
  { id:"lux6",   nom:"Luxury 32,50€", prixEur:32.50, transport:1.80, emballage:0.48 },
  { id:"lux7",   nom:"Luxury 35,50€", prixEur:35.50, transport:1.80, emballage:0.48 },
  { id:"lux8",   nom:"Luxury 38,75€", prixEur:38.75, transport:1.80, emballage:0.48 },
];
...
function PromoView({ onBack }) {
  ...
  const calc = (p) => {
    const totalEur  = p.prixEur + p.transport + p.emballage;
    const prixDA    = Math.round(totalEur * tx);
    const minDA     = Math.round(prixDA * 1.04);
    const promoDA   = parseFloat(promos[p.id]) || 0;
    const qte       = parseInt(qtes[p.id]) || 0;
    const marge     = promoDA > 0 && promoDA >= minDA ? Math.round((promoDA - prixDA) * qte) : 0;
    const ok        = promoDA >= minDA;
    return { totalEur, prixDA, minDA, promoDA, qte, marge, ok };
  };

  const totalMarge = PRODUITS_PROMO.reduce((s,p) => {
    const c = calc(p);
    return s + c.marge;
  }, 0);
      ...
      <div className="sb">
        ...
        <div className="card" style={{ marginBottom:14 }}>
          {[
            ...
            ["Transport",    produitActif.transport.toFixed(2)+"€"],
          ...
          <div style={{ display:"flex", justifyContent:"space-between", padding:"10px 0 4px" }}>
            ...
            <span style={{ fontSize:16, fontWeight:700, color:G }}>{calcActif.totalEur.toFixed(2)} €</span>
          </div>
          <div style={{ display:"flex", justifyContent:"space-between", padding:"4px 0" }}>
            <span style={{ fontSize:13, fontWeight:600 }}>Total PA (DA)</span>
            <span style={{ fontSize:16, fontWeight:700, color:G }}>{calcActif.prixDA.toLocaleString("fr-FR")} DA</span>
          </div>
        </div>

        {/* Prix minimum */}
        <p style={{ fontSize:11, color:MU, textTransform:"uppercase", letterSpacing:1, marginBottom:10 }}>🚫 Prix minimum (à ne pas dépasser vers le bas)</p>
        <div style={{ background:"rgba(74,222,128,.08)", border:"1.5px solid #4ade80", borderRadius:12, padding:"14px 16px", marginBottom:14, textAlign:"center" }}>
          <p style={{ fontSize:11, color:"#4ade80", marginBottom:4 }}>PA DA × 1,04 = marge minimale 4%</p>
          <p style={{ fontSize:30, fontWeight:700, color:"#4ade80" }}>{calcActif.minDA.toLocaleString("fr-FR")} DA</p>
        ...
        <div className="card" style={{ marginBottom:14 }}>
          <div style={{ marginBottom:12 }}>
            <label style={{ fontSize:11, color:MU, display:"block", marginBottom:5 }}>Mon prix promo (DA)</label>
            <input className="inp" type="number"
              placeholder={"Min : " + calcActif.minDA.toLocaleString("fr-FR") + " DA"}
              value={promos[produitActif.id]||""}
              onChange={e=>setPromos(p=>({...p,[produitActif.id]:e.target.value}))}
              style={{ borderColor: calcActif.promoDA > 0 ? (calcActif.ok?"#4ade80":"#e05050") : "rgba(255,255,255,.09)" }}
            />
            {calcActif.promoDA > 0 && !calcActif.ok && (
              <p style={{ fontSize:11, color:RD, marginTop:5 }}>⚠️ Prix trop bas ! Minimum : {calcActif.minDA.toLocaleString("fr-FR")} DA</p>
            )}
            {calcActif.promoDA > 0 && calcActif.ok && (
        ...
        {calcActif.promoDA > 0 && calcActif.ok && calcActif.qte > 0 && (
          <div style={{ background:"rgba(201,168,76,.1)", border:"1.5px solid " + G, borderRadius:12, padding:"14px 16px", textAlign:"center" }}>
            <p style={{ fontSize:12, color:MU, marginBottom:4 }}>({calcActif.promoDA.toLocaleString("fr-FR")} − {calcActif.prixDA.toLocaleString("fr-FR")}) × {calcActif.qte}</p>
            <p style={{ fontSize:28, fontWeight:700, color:G }}>{calcActif.marge.toLocaleString("fr-FR")} DA</p>
  ...
  return (
    <div className="fi">
      ...
      <div className="sb">
        ...
        {/* Total marge si des promos saisies */}
        {totalMarge > 0 && (
          <div style={{ background:"rgba(201,168,76,.1)", border:"1px solid "+G, borderRadius:12, padding:"12px 16px", marginBottom:14, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            ...
            <span style={{ fontSize:20, fontWeight:700, color:G }}>{totalMarge.toLocaleString("fr-FR")} DA</span>
          </div>
        )}
            ...
            <div key={p.id} onClick={()=>setSelected(p.id)}
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ display:"flex", gap:12, marginTop:3 }}>
                  <span style={{ fontSize:11, color:MU }}>PA : {c.prixDA.toLocaleString("fr-FR")} DA</span>
                  <span style={{ fontSize:11, color:"#4ade80" }}>Min : {c.minDA.toLocaleString("fr-FR")} DA</span>
                </div>
                {c.marge > 0 && (
                  <p style={{ fontSize:11, color:G, fontWeight:600, marginTop:3 }}>
                    Marge : {c.marge.toLocaleString("fr-FR")} DA ({c.qte} pcs)
                  </p>
                )}
              </div>
        ...
        <button className="btn-d" style={{ width:"100%", marginTop:8 }} onClick={()=>{ setPromos({}); setQtes({}); }}>↺ Tout réinitialiser</button>
      </div>
    </div>
  );
}
...
export default function ChoganApp() {
  ...
  const TABS = [
    ...
    { id:"catalogue",  lbl:"Inspirations", ic:"🌸" },
    { id:"commande",   lbl:"Commande",   ic:"🛒" },
    { id:"kit",        lbl:"Kit",        ic:"🎓" },
    { id:"catalogues", lbl:"Catalogues", ic:"📖" },
    { id:"checklist",  lbl:"Checklist",  ic:"✅" },
    { id:"formation",  lbl:"Formation",  ic:"📚" },
    { id:"ressources", lbl:"Ressources", ic:"📁" },
  ];
    ...
    <>
      ...
      <div className="app">
        <header className="hdr">
          ...
          <span className="hdr-sub">{TABS.find(t=>t.id===tab)?.lbl}</span>
        </header>
        ...
        <main className="main">
          {showBienvenue
            ...
            : <>
                {tab==="accueil"    && <AccueilView   started={started} setStarted={setStarted} />}
                {tab==="catalogue"  && <CatalogueView perfumes={perfumes} setPerfumes={setPerfumes} />}
                {tab==="commande"   && <BonCommandeView perfumes={perfumes} />}
                {tab==="kit"        && <KitDemarrageView />}
                ...
                {tab==="ressources" && <RessourcesView onOpenBienvenue={()=>setShowBienvenue(true)} onOpenPromo={()=>setShowPromo(true)} />}
              </>
          }
        </main>
